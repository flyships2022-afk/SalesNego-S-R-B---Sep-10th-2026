...
const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#161519] border border-white/15 rounded-xl p-3.5 shadow-xl text-white text-xs space-y-1.5 min-w-[200px] z-50">
...
export const MarketSignalChart: React.FC = () => {
  ...
  return (
    <div
      ...
      className="w-full bg-[#161519] dark:bg-[#1C1B20] border border-[#E5E3DC]/30 dark:border-white/10 rounded-2xl p-5 sm:p-8 shadow-md"
    >
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-white/10 pb-5">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#EE2338]/15 border border-[#EE2338]/30 text-[#EE2338] text-xs font-semibold mb-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
          <span className="text-[11px] text-zinc-400 block font-medium">Avg Signal Index</span>
          <span className="text-lg sm:text-xl font-extrabold text-white">84.5<span className="text-xs text-zinc-400 font-normal"> / 100</span></span>
        </div>
        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
          <span className="text-[11px] text-zinc-400 block font-medium">Highest Velocity</span>
          <span className="text-lg sm:text-xl font-extrabold text-[#EE2338]">RFP / Intent</span>
        </div>
        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
          <span className="text-[11px] text-zinc-400 block font-medium">Conversion Lift</span>
          <span className="text-lg sm:text-xl font-extrabold text-white">+3.4x</span>
        </div>
        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
          <span className="text-[11px] text-zinc-400 block font-medium">Data Confidence</span>
          <span className="text-lg sm:text-xl font-extrabold text-[#EE2338]">94.2%</span>
        </div>
      </div>
          ...
          {viewMode === 'category' ? (
            ...
            >
              ...
              <Legend
                ...
                formatter={(value) => <span className="text-zinc-200 text-xs font-medium">{value}</span>}
              />
              <Bar
              ...
              >
                {categoryData.map((entry, index) => (
                  <Cell
                    key={`cell-sig-${index}`}
                    fill={entry.signalStrength >= 90 ? '#EE2338' : '#C91521'}
                  />
                ))}
              </Bar>
              <Bar
      ...
      {/* Strategy Footnote */}
      <div className="mt-4 pt-3 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs text-zinc-400 gap-2">
        <div className="flex items-center gap-1.5">
          <Info className="w-3.5 h-3.5 text-[#EE2338] shrink-0" />
          <span>Calculated from multi-source ICP trigger scans, intent data, and conversion post-mortems.</span>
        </div>
        <span className="text-zinc-300 font-mono text-[11px]">Updated Quarterly</span>
      </div>
    </div>
  );
};
